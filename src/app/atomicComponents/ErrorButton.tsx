...
const Button = styled.div<{
  ...
  display: flex;
  flex-direction: ${({ isReverseDirection }) =>
    isReverseDirection ? 'row-reverse' : 'row'};
  ...
  padding: 0 18px;
  font-weight: 600;
  border: ${({ isLoading, theme, isDisabled }) =>
    !isDisabled &&
    `1px solid ${isLoading ? theme.colors.primaryErrorRegular : theme.colors.primaryError}`};
  color: ${({ theme, isDisabled }) => {
    return isDisabled ? theme.colors.gray40 : theme.colors.primaryError;
  }};
  cursor: ${({ isDisabled }) => (isDisabled ? 'default' : 'pointer')};
  background-color: ${({ theme, isDisabled }) =>
    isDisabled ? theme.colors.gray20 : 'rgba(236, 28, 38, 0.03)'};
  :hover {
    background-color: ${({ isDisabled, isLoading }) =>
      !isDisabled && !isLoading && 'rgba(236, 28, 38, 0.1)'};
  }
`;
