...
import {
  checkAllowance,
  requestAllowance
} from '~root/services/tokenContract.service';
...
const Step = styled.div<{ isCurrent: boolean; hasFinished?: boolean }>`
  ...
  background-color: ${({ theme, isCurrent }) =>
    isCurrent
      ? theme.colors.primarySuccessRegularOpacity
      : theme.colors.gray10};
  border: ${({ theme, isCurrent }) =>
    `1px solid ${isCurrent ? theme.colors.primarySuccessDark : theme.colors.gray30}`};
  ${({ hasFinished }) => {
    if (hasFinished) {
      return {
        border: 'none',
      ...
      <Container>
        <ButtonWrapper>
          <PrimaryButton
            ...
            onClick={
              !allowanceButtonDisable ? requestAllowanceHandler : () => {}
            }
            size={ButtonSize.XL}
          />
          <PrimaryButton
  ...
  return (
    <Grid container>
      {checkboxText && checkBoxCallBack && (
        <Grid item xs={12}>
          <CheckBox
            toggleIsChecked={checkBoxCallBack}
            text={checkboxText}
