...
const LiquidationStateError = (props: Props) => {
  ...
  const renderText = () => {
      ...
      return (
        <div>
          ...
          <LinkText
            ...
            link={config.links.ETHER_RESPONSIBILITIES}
          />
    ...
    if (errorType === LiquidationStateErrorType.Deposit) {
      return (
        'Your balance is running low and puts your cluster at risk. \n' +
        'To avoid liquidation please deposit more funds to your cluster.'
      );
    }
    if (errorType === LiquidationStateErrorType.Withdraw) {
      return (
        'This withdrawal amount will putting your cluster at risk of liquidation.\n' +
        'To avoid liquidation please withdraw less funds from your cluster.\n'
      );
    }
    const firstText =
      errorType === LiquidationStateErrorType.ChangeOperatorsLiquidationWarning
        ? 'This withdrawal amount will place your cluster at risk of liquidation.'
        : 'This fee change will liquidate your cluster, please deposit more SSV or';
    const secondText =
      errorType === LiquidationStateErrorType.ChangeOperatorsLiquidationWarning
        ? 'To avoid liquidation please withdraw less funds from your cluster.'
        : 'change to a different set of operators.';

    if (
      errorType ===
        LiquidationStateErrorType.ChangeOperatorsLiquidationWarning ||
      errorType === LiquidationStateErrorType.ChangeOperatorsLiquidation
    ) {
      return (
        <Grid container item className={classes.OperatorChangeTextWrapper}>
          <Grid item>
            <Typography className={classes.OperatorChangeText}>
              {firstText}
            </Typography>
            <Typography className={classes.OperatorChangeText}>
              {secondText}
          ...
          <Grid item>
            <LinkText
              ...
              withoutUnderline
              text={'Deposit'}
              link={config.routes.SSV.MY_ACCOUNT.CLUSTER.DEPOSIT}
            />
          </Grid>
        </Grid>
      );
