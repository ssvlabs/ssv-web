      ...
      <Grid container item className={classes.SectionWrapper}>
        <Grid container item className={classes.Header} xs={12}>
          ...
          {cluster.isLiquidated && (
            <Grid className={classes.Liquidated}>Liquidated</Grid>
          )}
          {cluster.runWay < 30 &&
            !cluster.isLiquidated &&
            !!Number(cluster.balance) &&
        ...
        <Grid container item>
          {cluster.balance || cluster.isLiquidated ? (
            <Grid
              ...
              className={
                cluster.runWay < 30
                  ? classes.CurrentBalanceLiquidated
      ...
      {(Number(cluster.balance) || cluster.isLiquidated) && (
        <OperationRunwayWrapper>
          ...
          {cluster.isLiquidated ? (
            ...
            >
              <PrimaryButton
                text={'Reactivate Cluster'}
                onClick={moveToReactivateCluster}
                size={ButtonSize.XL}
              />
            </Grid>
          ) : (
