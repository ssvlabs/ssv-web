...
const ClusterDashboard = () => {
  ...
  const { page, pages, per_page, total } = useAppSelector(
    getClustersPagination
  );
  ...
  useEffect(() => {
    ...
    clusterIntervalRef.current = setInterval(
  ...
  const rows = sortedClusters.map((cluster: any) => {
    ...
    const remainingDays =
      cluster.runWay && cluster.runWay !== Infinity
        ? `${remainingDaysValue} Days`
        : remainingDaysValue;
    return createData(
      longStringShorten(
        getClusterHash(cluster.operators, accountAddress).slice(2),
        4
      ),
        ...
        {cluster.operators.map((operator: any, index: number) => {
          return (
            ...
            >
              {hoveredGrid ===
                getClusterHash(cluster.operators, accountAddress) +
                  operator.id && <OperatorCard operator={operator} />}
      ...
      isNaN(cluster.runWay) ? (
        <NaDisplay
          text={translations.NA_DISPLAY.TOOLTIP_TEXT}
          tooltipClassExtend={classes.TooltipCustomSize}
        />
      ) : (
        remainingDays
      ),
  ...
  const rowBackgroundColor = (index: number) => {
    ...
    if (indexCluster.runWay < 30)
      return isDarkMode ? 'rgba(255, 210, 10, 0.2)' : '#FDFBF0';
  };
    ...
    <Grid container className={classes.MyAccountWrapper}>
      <Grid container item className={classes.HeaderWrapper}>
        ...
        <Grid container item xs className={classes.HeaderButtonsWrapper}>
          {rows.length > 0 && (
            <SecondaryButton
              onClick={moveToFeeRecipient}
              icon={'/images/test/Group.svg'}
              text={'Fee Address'}
              size={ButtonSize.MD}
            />
          )}
          <PrimaryButton
            onClick={moveToRegisterValidator}
            text={'Add Cluster'}
            size={ButtonSize.MD}
          />
        </Grid>
        ...
        columns={[
          {
            ...
            tooltip: (
              <Grid>
                ...
                validators.{' '}
                <LinkText
                  text={'Read more on clusters'}
                  link={config.links.MORE_ON_CLUSTERS}
                />
              </Grid>
            )
          },
