...
const ClusterWarnings = ({ cluster }: { cluster: any }) => {
  ...
  if (cluster.isLiquidated) {
    return (
      <Grid className={classes.Liquidated} onMouseLeave={handleWarningLeave} onMouseEnter={handleWarningHover}>
        Liquidated
        {showPopUp && (
  ...
  if (cluster.runWay < 30) {
    return (
      <Grid className={classes.LowRunWay} onMouseLeave={handleWarningLeave} onMouseEnter={handleWarningHover}>
        Low Runway
        {showPopUp && (
