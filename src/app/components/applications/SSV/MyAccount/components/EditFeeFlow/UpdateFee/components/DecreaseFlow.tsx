...
import {
  decreaseOperatorFee,
  getOperatorBalance
} from '~root/services/operatorContract.service';
    ...
    <BorderScreen
      ...
      body={[
        <Grid container className={classes.ChangeFeeWrapper}>
          <Typography>
            {updated
              ? 'You have successfully updated your fee. The new fee will take effect immediately.'
              : 'Your new operator annual fee will be updated to.'}
          </Typography>
          ...
          {!updated && (
            <Grid item className={classes.Notice}>
              <Grid item className={classes.BulletsWrapper}>
                {Number(newFee) === 0 ? (
                  <div>
                    Please note that operators who have set their fee to 0 will
                    not have the option to increase or modify their fee in the
                    future.'
                  </div>
                ) : (
                  <div>
                    Keep in mind that the process of increasing your fee is
                    different than decreasing it, and returning back to your
                    current fee in the future would take longer. Read more on{' '}
                    <LinkText
