...
const ReactStepper = ({
  ...
  const time =
    (operatorFeeData.operatorApprovalBeginTime -
      feeIncreaseAndPeriods.declaredOperatorFeePeriod) *
    1000;
  ...
  const getStepNumber = (currentStep: number, currentPosition: number) =>
    currentStep === currentPosition - 1 || currentStep >= currentPosition
      ? ''
      : currentPosition;
      ...
      <Grid item container className={classes.ProgressBarTextWrapper}>
        <Grid className={classes.ProgressBarText}>
          ...
          <Typography className={classes.DeclaredFee}>
            {step === StepperProcessesSteps.DECLARE_STEP
              ? subText
              : declareFeeDate.toLocaleString('en-US', options)}
          </Typography>
        </Grid>
        <Grid className={classes.ProgressBarText}>
          Waiting Period
          {step === StepperProcessesSteps.WAITING_STEP && (
            <Typography className={classes.WaitingPeriod}>{subText}</Typography>
          )}
        </Grid>
        <Grid className={classes.ProgressBarText}>
          Pending Execution
          {step === StepperProcessesSteps.PENDING_STEP && (
            <Typography className={classes.ExpiresIn}>{subText}</Typography>
          )}
        </Grid>
