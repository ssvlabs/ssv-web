...
const CountriesAutocompleteInput = ({
  fieldKey,
  placeholder
}: {
  fieldKey: string;
  ...
  useEffect(
    ...
    [metadataStore.locationsData.toString()]
  );
  ...
  const customFilterOptions = (_options: any, state: any) => {
    ...
    return metadataStore.locationsData
      .filter(
        (d) =>
          d.name.toLowerCase().includes(inputValue) ||
          d['alpha-3'].toLowerCase().includes(inputValue)
      )
  ...
  const onTagsChange = (event: any, value: any) => {
    if (event) {
      ...
      metadataStore.setMetadataValue(
        fieldKey,
        metadataStore.locationsList[value]
      );
    }
  };

  const countryWithAlpha = (location: string) => {
    const country = metadataStore.locationsData.find(
      (c) => c.name === location
    );
  ...
  return (
    <Autocomplete
      ...
      options={metadataStore.locationsData.map(
