  ...
  const submitHandler = async () => {
    ...
    if (!isNotValidity) {
      ...
      fieldsToValidateSignature.forEach((field) => {
        if (payload[field]) {
          const newItem =
            field === FIELD_KEYS.OPERATOR_IMAGE
              ? `logo:sha256:${sha256(payload[field])}`
              : payload[field];
      ...
      try {
        ...
        setErrorMessage([
          'You must confirm the signature request through your wallet'
        ]);
        setIsLoading(false);
        return;
      }
      const updateOperatorResponse = await updateOperatorMetadata(
        operator.id,
        signatureHash,
        payload,
        isContractWallet
      ...
      if (updateOperatorResponse.data) {
        ...
        setErrorMessage([
          updateOperatorResponse.error || 'Update metadata failed'
        ]);
      }
      setIsLoading(false);
    }
