...
const ReactivateCluster = () => {
  const [customPeriod, setCustomPeriod] = useState(
    config.GLOBAL_VARIABLE.DEFAULT_CLUSTER_PERIOD
  );
  ...
  const totalCost = new Decimal(operatorsCost)
    .add(networkCost)
    .add(liquidationCollateralCost)
    .mul(validatorsCount);
  const insufficientBalance = totalCost.comparedTo(walletSsvBalance) === 1;
  const showLiquidationError =
    isCustomPayment && !insufficientBalance && timePeriodNotValid;
  const disableCondition =
    insufficientBalance || customPeriod <= 0 || isNaN(customPeriod);
        ...
        body={[
          <Grid container>
            <Typography className={classes.Text}>
              ...
              <LinkText
                withoutUnderline
                text={'liquidated'}
                link={config.links.REACTIVATION_LINK}
              />{' '}
              due to insufficient balance for its operational
              <br /> costs. To resume its operation, you must deposit sufficient
              funds required for
              <br /> its reactivation.{' '}
              <LinkText
                withoutUnderline
            ...
            <Typography className={classes.BoxesHeader}>
              Select your cluster funding period
            </Typography>
            <Grid container item style={{ gap: 16 }}>
              {options.map((option, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isChecked(option.id) ? (
                        <Grid item className={classes.CheckedCircle} />
                      ) : (
                    ...
                    <Grid item className={classes.SsvPrice}>
                      {formatNumberToUi(
                        propertyCostByPeriod(
                          operatorsFee,
                          isCustom ? customPeriod : option.days
                        ) * validatorsCount
                      )}{' '}
                      SSV
                    </Grid>
                    {isCustom && (
                      <TextInput
                        ...
                        onChangeCallback={(e: any) =>
                          setCustomPeriod(Number(e.target.value))
                        }
                ...
                <ErrorMessage
                  ...
                  text={
                    <Grid container style={{ gap: 8 }}>
                      <Grid item>
                        Insufficient SSV balance. Acquire further SSV or pick a
                        different amount.
                      </Grid>
              ...
              {showLiquidationError && (
                <ErrorMessage
                  ...
                  text={
                    <Grid>
                      This funding period will put your validator at risk of
                      liquidation. To avoid liquidation please pick a bigger
                      funding period.{' '}
                      <LinkText
          ...
          <Grid container>
            ...
            <AllowanceButton
              ...
              totalAmount={
                disableCondition ? '0' : formatNumberToUi(totalCost.toFixed(18))
              }
            />
          </Grid>
        ]}
