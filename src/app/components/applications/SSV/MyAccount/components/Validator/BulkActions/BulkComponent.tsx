...
import {
  exitValidators,
  removeValidators
} from '~root/services/validatorContract.service';
import {
  getSelectedCluster,
  setExcludedCluster
...
const BULK_FLOWS_ACTION_TITLE = {
  [BULK_FLOWS.BULK_REMOVE]:
    translations.VALIDATOR.REMOVE_EXIT_VALIDATOR.BULK_TITLES
      .SELECT_REMOVE_VALIDATORS,
  [BULK_FLOWS.BULK_EXIT]:
    translations.VALIDATOR.REMOVE_EXIT_VALIDATOR.BULK_TITLES
      .SELECT_EXIT_VALIDATORS
};

const MAX_VALIDATORS_COUNT =
  Number(window.localStorage.getItem(MAXIMUM_VALIDATOR_COUNT_FLAG)) || 100;

const BULK_ACTIONS_TOOLTIP_TITLES = {
  [BULK_FLOWS.BULK_REMOVE]:
    translations.VALIDATOR.REMOVE_EXIT_VALIDATOR.BULK_TOOLTIPS.REMOVE_VALIDATORS(
      MAX_VALIDATORS_COUNT
    ),
  [BULK_FLOWS.BULK_EXIT]:
    translations.VALIDATOR.REMOVE_EXIT_VALIDATOR.BULK_TOOLTIPS.EXIT_VALIDATORS(
      MAX_VALIDATORS_COUNT
    )
};
...
const BULK_ACTIONS_TOOLTIP_CHECKBOX_TITLES = {
  [BULK_FLOWS.BULK_REMOVE]:
    translations.VALIDATOR.REMOVE_EXIT_VALIDATOR.BULK_TOOLTIPS.REMOVE_VALIDATORS_CHECKBOX(
      MAX_VALIDATORS_COUNT
    ),
  [BULK_FLOWS.BULK_EXIT]:
    translations.VALIDATOR.REMOVE_EXIT_VALIDATOR.BULK_TOOLTIPS.EXIT_VALIDATORS_CHECKBOX(
      MAX_VALIDATORS_COUNT
    )
};
...
const BULK_FLOWS_CONFIRMATION_DATA = {
  [BULK_FLOWS.BULK_REMOVE]:
    translations.VALIDATOR.REMOVE_EXIT_VALIDATOR.FLOW_CONFIRMATION_DATA.REMOVE,
  [BULK_FLOWS.BULK_EXIT]:
    translations.VALIDATOR.REMOVE_EXIT_VALIDATOR.FLOW_CONFIRMATION_DATA.EXIT
};

const BulkComponent = () => {
  const [selectedValidators, setSelectedValidators] = useState<
    Record<string, BulkValidatorData>
  >({});
  ...
  const selectMaxValidatorsCount = (
    validators: IValidator[],
    validatorList: Record<string, BulkValidatorData>
  ): Record<string, BulkValidatorData> => {
    const isSelected = Object.values(selectedValidators).every(
      (validator: { validator: IValidator; isSelected: boolean }) =>
      ...
      if (selectAll) {
        ...
        validators.forEach((validator: IValidator) => {
          validatorList[formatValidatorPublicKey(validator.public_key)] = {
            validator,
            isSelected:
              selectedValidators[formatValidatorPublicKey(validator.public_key)]
                ?.isSelected || false
          };
        });
      }
  ...
  const backToSingleClusterPage = (validatorsCount?: number) => {
    ...
    navigate(
      validatorsCount === cluster.validatorCount && cluster.isLiquidated
        ? -2
        : -1
    );
  };
  ...
  const nextStep = async () => {
    ...
    let res;
    const selectedValidatorsCount = selectedValidatorValues.filter(
      (validator) => validator.isSelected
    ).length;
    const condition = selectedValidatorsCount > 1;
    if (currentStep === BULK_STEPS.BULK_ACTIONS) {
      setCurrentStep(BULK_STEPS.BULK_CONFIRMATION);
    } else if (
      currentStep === BULK_STEPS.BULK_CONFIRMATION &&
      currentBulkFlow === BULK_FLOWS.BULK_EXIT
    ) {
      setIsLoading(true);
      const validatorIds = condition
        ? selectedValidatorKeys.filter(
            (publicKey: string) => selectedValidators[publicKey].isSelected
          )
        : formatValidatorPublicKey(
            selectedValidatorValues.filter(
              (selectedValidator) => selectedValidator.isSelected
            )[0].validator.public_key
          );
      res = await exitValidators({
        isContractWallet,
        validatorIds,
        operatorIds: process.item.operators.map(
      ...
      setIsLoading(true);
      if (
        selectedValidatorsCount === cluster.validatorCount &&
      ...
      res = await removeValidators({
        ...
        isContractWallet,
        validatorPks,
        operators: process.item.operators,
  ...
  if (currentStep === BULK_STEPS.BULK_ACTIONS && !process.validator) {
    return (
      <NewBulkActions
        nextStep={nextStep}
        tooltipTitle={
          BULK_ACTIONS_TOOLTIP_TITLES[currentBulkFlow ?? BULK_FLOWS.BULK_REMOVE]
        }
        checkboxTooltipTitle={
          BULK_ACTIONS_TOOLTIP_CHECKBOX_TITLES[
            currentBulkFlow ?? BULK_FLOWS.BULK_REMOVE
          ]
        }
        maxValidatorsCount={MAX_VALIDATORS_COUNT}
        title={
          BULK_FLOWS_ACTION_TITLE[currentBulkFlow ?? BULK_FLOWS.BULK_REMOVE]
        }
  ...
  if (currentStep === BULK_STEPS.BULK_CONFIRMATION) {
    return (
      <ConfirmationStep
        stepBack={!process.validator ? stepBack : undefined}
        flowData={
          BULK_FLOWS_CONFIRMATION_DATA[
            currentBulkFlow ?? BULK_FLOWS.BULK_REMOVE
          ]
        }
        selectedValidators={Object.keys(selectedValidators).filter(
          (publicKey: string) => selectedValidators[publicKey].isSelected
        )}
        isLoading={isLoading}
