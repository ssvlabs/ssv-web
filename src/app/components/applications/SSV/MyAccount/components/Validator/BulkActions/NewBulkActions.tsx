...
const NewBulkActions = ({
  ...
  const selectedValidatorsCount = validatorsListArray.filter(
    (validator: BulkValidatorData) => validator.isSelected
  ).length;
  const totalCount =
    process.item.validatorCount > maxValidatorsCount
      ? maxValidatorsCount
      : process.item.validatorCount;
  ...
  const tooltipTitleComponent = (tooltipText: string) =>
    validatorsListArray.length > maxValidatorsCount ? (
      <TooltipTitleWrapper>
        {tooltipText}
        <TooltipLink onClick={createValidatorsLaunchpad}>
    ...
    <Wrapper>
      ...
      <ValidatorsWrapper>
        <HeaderWrapper>
            ...
            {showIndicatorCondition &&
              (isLoading ? (
                ...
                <AnchorTooltip
                  title={
                    validatorsListArray.length > maxValidatorsCount
                      ? tooltipTitleComponent(tooltipTitle)
                      : null
                  }
                  placement={'top'}
                >
          ...
          {showSubHeaderCondition && (
            <SubHeader>Select up to {maxValidatorsCount} validators</SubHeader>
          )}
          <ValidatorsList
            withoutSettings
