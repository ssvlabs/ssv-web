...
const FundingNewValidator = () => {
  ...
  const newValidatorsCount = validatorStore.validatorsCount
    ? validatorStore.validatorsCount
    : 1;
  const newBurnRate = getClusterNewBurnRate(
    cluster.operators,
    cluster.validatorCount + newValidatorsCount,
    networkFee
  );
  ...
  const calculateNewRunWayCondition =
    checkedId === OPTION_DEPOSIT_ADDITIONAL_FUNDS
      ? Number(depositSSV) > 0
      : true;
  const runWay =
    checkedId === OPTION_USE_CURRENT_BALANCE ||
    (checkedId === OPTION_DEPOSIT_ADDITIONAL_FUNDS && Number(depositSSV) > 0)
  ...
  const daysChanged = () => {
    if (!isFinite(cluster.runWay))
      return (
        <Typography className={classes.PositiveDays}>
          (+{formatNumberToUi(newRunWay, true)} days)
        </Typography>
      );
    if (cluster.runWay > newRunWay)
      return (
        <Typography className={classes.NegativeDays}>
          (-{formatNumberToUi(cluster.runWay - newRunWay, true)} days)
        </Typography>
      );
    return (
      <Typography className={classes.PositiveDays}>
        (+{formatNumberToUi(newRunWay - cluster.runWay, true)} days)
      </Typography>
    );
  };

  const ssvChanged = () => {
    if (Number(depositSSV) > 0)
      return (
        <Typography className={classes.PositiveDays}>
          (+{depositSSV} SSV)
        </Typography>
            ...
            <Grid container item>
              ...
              >
                ...
                <Grid container item style={{ gap: 8 }}>
                  ...
                  <Typography className={`${classes.Bold} ${classes.LessBold}`}>
                    {ssvChanged()}
                  </Typography>
                </Grid>
              </Grid>
              ...
              >
                <Grid container item alignItems={'center'} style={{ gap: 10 }}>
                  <Typography className={classes.LightGreyHeader}>
                    Est. Operational Runway
                  </Typography>
                  <ToolTip
                    classExtend={classes.ToolTip}
                    text={
                      'Estimated amount of days the cluster balance is sufficient to run all it’s validator'
                    }
                  />
                </Grid>
                <Grid container item style={{ gap: 8, alignItems: 'center' }}>
                  ...
                  {checkedId > 0 && calculateNewRunWayCondition && (
                    <Typography
                      className={`${classes.Bold} ${classes.LessBold}`}
                    >
                      {daysChanged()}
                    </Typography>
                  )}
                      ...
                      <Grid container style={{ gap: 8 }}>
                        ...
                        <Grid container item xs>
                          <LinkText
                            ...
                            text={errorMessage.link.text}
                            link={errorMessage.link.path}
                          />
                        </Grid>
                      </Grid>
                    }
              ...
              <Grid container style={{ marginTop: 24 }}>
                <PrimaryButton
                  isDisabled={disableBtnCondition}
                  text={'Next'}
