...
const Settings = ({
  ...
  const openExplorer = (publicKey: string) => {
    ...
    window.open(
      `${config.links.EXPLORER_URL}/validators/${publicKey}`,
      '_blank'
    );
  };
  ...
  return (
    <Grid container className={classes.ExtraButtonsWrapper}>
      ...
      />
      <ImageDiv
        onClick={() => openExplorer(validator.public_key)}
        image={'explorer'}
        width={24}
        height={24}
      />
        ...
        <Grid item className={classes.SettingsWrapper}>
          <Grid ref={settingsRef} container item className={classes.Settings}>
            <Grid
              ...
              onClick={() => openLink(config.links.UPDATE_OPERATORS_LINK)}
            ...
            <Grid
              ...
              item
              className={classes.Button}
              onClick={() => moveToRemoveValidator(BULK_FLOWS.BULK_REMOVE)}
            >
