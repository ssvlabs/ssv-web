...
const OperatorAccessSettings = () => {
  ...
  const [isPermissionedOperator, setIsPermissionedOperator] = useState(
    isOperatorPermissioned
  );
  const isFirstUsage =
    !operator?.address_whitelist &&
    address === config.GLOBAL_VARIABLE.DEFAULT_ADDRESS_WHITELIST;
  const btnDisabledCondition =
    addressError.shouldDisplay ||
    !address ||
    isFirstUsage ||
    address.toString() === operator.address_whitelist?.toString();
        ...
        <Grid container item style={{ gap: 14 }}>
          <Grid className={classes.HeaderWrapper} container style={{ gap: 16 }}>
            <Grid className={classes.HeaderInner}>
              <Typography className={classes.InfoText}>
                {translations.OPERATOR_WHITELIST_ADDRESS.SECOND_TITLE}
              </Typography>
              <Tooltip
                text={
                  <Grid>
                    ...
                    <LinkText
                      textSize={12}
                      text={'Permissioned Operators'}
                      link={config.links.SSV_DOCUMENTATION}
                    />
                  </Grid>
                }
              />
            ...
            <Switch
              checked={isPermissionedOperator}
          ...
          <Grid container gap={{ gap: 17 }}>
            <Typography className={classes.Text}>
              {translations.OPERATOR_WHITELIST_ADDRESS.TEXT}
            </Typography>
            {isPermissionedOperator && (
              <Grid item container>
                <Grid className={classes.InputLabelWrapper}>
                  <InputLabel
                    title={translations.OPERATOR_WHITELIST_ADDRESS.INPUT_LABEL}
                  />
                  <Tooltip
                    text={
                      translations.OPERATOR_WHITELIST_ADDRESS
                        .INPUT_LABEL_TOOLTIP
                    }
                  />
                </Grid>
            ...
            >
              <PrimaryButton
                isDisabled={btnDisabledCondition}
                text={'Update'}
