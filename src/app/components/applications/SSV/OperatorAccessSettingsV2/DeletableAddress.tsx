...
type FCProps = FC<Omit<ComponentPropsWithoutRef<'div'>, keyof DeletableAddressProps> & DeletableAddressProps>;
...
export const DeletableAddress: FCProps = ({ className, address, onDelete, onUndo, isMarked, disabled, ...props }) => {
  return (
    ...
    >
      ...
      {!disabled &&
        (isMarked ? (
          <Button variant="ghost" className="text-primary-500" onClick={() => onUndo(address)}>
            Undo
          </Button>
        ) : (
