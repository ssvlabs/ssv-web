...
const OperatorConfirmation = () => {
  ...
  const onRegisterClick = async () => {
    ...
    if (operatorAdded) {
      navigate(isContractWallet ? config.routes.SSV.MY_ACCOUNT.OPERATOR_DASHBOARD : config.routes.SSV.OPERATOR.SUCCESS_PAGE, { state: { operatorRawData }});
    } else {
          ...
          <Grid container style={{ gap: 34 }}>
            ...
            <Grid container item>
              ...
              <Grid item xs={6} className={classes.AlignRight}>
                <NameAndAddress
                  name={`0x${longStringShorten(operatorRawData.address.substring(2), 4)}`}/>
              </Grid>
            </Grid>
          </Grid>
        </Grid>,
        <Grid container style={{ gap: 24 }}>
          <Grid container item style={{ gap: 16 }}>
            ...
            <Grid container item>
              ...
              <Grid item xs={6} className={classes.AlignRight}>
                <SsvAndSubTitle
                  ssv={formatNumberToUi(operatorRawData.fee)}
                  subText={'/year'}
                />
              </Grid>
            </Grid>
