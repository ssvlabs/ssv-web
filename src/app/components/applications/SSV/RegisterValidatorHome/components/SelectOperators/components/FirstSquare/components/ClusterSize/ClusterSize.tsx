...
const ClusterSize = ({ currentClusterSize, changeClusterSize }: {
  currentClusterSize: number,
  changeClusterSize: Function
}) => {
  ...
  const clusterSizeHandler = (clusterSize: number) => {
    ...
    for (const key of [...selectedOperators.values()]) {
  ...
  return (
    <Grid className={classes.ClusterSizeWrapper}>
      ...
      {sizes.map((size: number, index: number) => <Grid key={`${size}${index}`} onClick={() => clusterSizeHandler(size)}
                                                        className={`${classes.ClusterSizeButton} ${size === currentClusterSize && classes.ChosenClusterSize}`}>{size}</Grid>)}
    </Grid>
  );
};
