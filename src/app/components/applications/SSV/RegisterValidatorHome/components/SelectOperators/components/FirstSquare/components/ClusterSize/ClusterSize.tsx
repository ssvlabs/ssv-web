...
const ClusterSize = ({ currentClusterSize, changeClusterSize }: { currentClusterSize: number, changeClusterSize: Function }) => {
    ...
    return (
        <Grid className={classes.ClusterSizeWrapper}>
            ...
            {sizes.map((size: number) => <Grid onClick={() => clusterSizeHandler(size)} className={`${classes.ClusterSizeButton} ${size === currentClusterSize && classes.ChosenClusterSize}`}>{size}</Grid>)}
        </Grid>
    );
};
