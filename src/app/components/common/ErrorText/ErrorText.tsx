...
const ErrorText = (props: Props) => {
  ...
  const renderText = () => {
    if (errorType === ErrorType.WithdrawAll) {
      return (
        <div>
          Withdrawing the requested amount will liquidate your account, which
    ...
    if (errorType === ErrorType.Deposit) {
      return (
        'Your balance is running low and puts your account at risk. \n' +
        'To avoid liquidation please deposit more funds to your account.'
      );
    }
    if (errorType === ErrorType.Withdraw) {
      return (
        'This withdrawal amount will putting your account at risk of liquidation.\n' +
