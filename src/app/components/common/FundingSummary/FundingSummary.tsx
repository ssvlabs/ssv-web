...
const FundingSummary = (props: Props) => {
  ...
  const isMultiSharesMode =
    validatorStore.isMultiSharesMode ||
    (props.validatorsCount && props.validatorsCount > 1);
  const countOfValidators =
    props.validatorsCount || validatorStore.validatorsCount;
  ...
  const networkCost =
    props.networkCost ?? propertyCostByPeriod(networkFee, daysPeriod);
  const operatorsCost =
    props.operatorsCost ??
    propertyCostByPeriod(selectedOperatorsFee, daysPeriod);
  ...
  const mandatoryColumns = isMultiSharesMode
    ...
    : Object.values(FundingSummeryColumns).filter(
        (flow: string) =>
          flow !== FundingSummeryColumns.FEE &&
          flow !== FundingSummeryColumns.VALIDATORS
      );

  const columnValues = {
    [FundingSummeryColumns.FUNDING_SUMMARY]: (value: number) =>
      payments.find((payment) => payment.id === value)?.name,
    [FundingSummeryColumns.FEE]: (value: number) =>
      `${formatNumberToUi(paymentsValue(value))} SSV`,
  ...
  return (
    <Grid className={classes.Wrapper}>
      {mandatoryColumns.map((fundingSummeryColumnName: string) => (
        <Grid
          className={columnStyles[fundingSummeryColumnName]}
          key={fundingSummeryColumnName}
        >
          <Typography className={classes.Title}>
            {fundingSummeryColumnName}
          </Typography>
