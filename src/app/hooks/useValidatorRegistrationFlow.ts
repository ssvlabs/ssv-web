...
import {
  getLocalStorageFlagValue,
  MAXIMUM_VALIDATOR_COUNT_FLAG
} from '~lib/utils/developerHelper';
...
const MAX_VALIDATORS_PER_CLUSTER_SIZE: Record<number, number> = {
  [config.GLOBAL_VARIABLE.CLUSTER_SIZES.QUAD_CLUSTER]:
    config.GLOBAL_VARIABLE.FIXED_VALIDATORS_COUNT_PER_CLUSTER_SIZE.QUAD_CLUSTER,
  [config.GLOBAL_VARIABLE.CLUSTER_SIZES.SEPT_CLUSTER]:
    config.GLOBAL_VARIABLE.FIXED_VALIDATORS_COUNT_PER_CLUSTER_SIZE.SEPT_CLUSTER,
  [config.GLOBAL_VARIABLE.CLUSTER_SIZES.DECA_CLUSTER]:
    config.GLOBAL_VARIABLE.FIXED_VALIDATORS_COUNT_PER_CLUSTER_SIZE.DECA_CLUSTER,
  [config.GLOBAL_VARIABLE.CLUSTER_SIZES.TRISKAIDEKA_CLUSTER]:
    config.GLOBAL_VARIABLE.FIXED_VALIDATORS_COUNT_PER_CLUSTER_SIZE
      .TRISKAIDEKA_CLUSTER
};
...
const BULK_MODE_TO_ROUTES: NavigationRoutes = {
  [config.routes.SSV.MY_ACCOUNT.CLUSTER.ROOT]:
  ...
  [config.routes.SSV.MY_ACCOUNT.CLUSTER.DISTRIBUTION_METHOD_START]: {
    ...
    [EValidatorFlowAction.GENERATE_KEY_SHARES_ONLINE]:
      config.routes.SSV.MY_ACCOUNT.CLUSTER.UPLOAD_KEY_STORE,
    [EValidatorFlowAction.GENERATE_KEY_SHARES_OFFLINE]:
      config.routes.SSV.MY_ACCOUNT.CLUSTER.DISTRIBUTE_OFFLINE
  },
  [config.routes.SSV.MY_ACCOUNT.CLUSTER_DASHBOARD]:
    config.routes.SSV.VALIDATOR.HOME,
  [config.routes.SSV.VALIDATOR.HOME]: {
    [EValidatorFlowAction.GENERATE_NEW_SHARE]:
      config.routes.SSV.VALIDATOR.SELECT_OPERATORS,
    [EValidatorFlowAction.ALREADY_HAVE_SHARES]:
      config.routes.SSV.MY_ACCOUNT.CLUSTER.UPLOAD_KEYSHARES
  },
  [config.routes.SSV.VALIDATOR.SELECT_OPERATORS]:
  ...
  [config.routes.SSV.MY_ACCOUNT.CLUSTER.UPLOAD_KEYSHARES]: {
    [EValidatorFlowAction.FIRST_REGISTER]:
      config.routes.SSV.VALIDATOR.FUNDING_PERIOD_PAGE,
    [EValidatorFlowAction.SECOND_REGISTER]:
      config.routes.SSV.MY_ACCOUNT.CLUSTER.ADD_VALIDATOR
  },
  [config.routes.SSV.MY_ACCOUNT.CLUSTER.UPLOAD_KEY_STORE]: {
    [EValidatorFlowAction.FIRST_REGISTER]:
      config.routes.SSV.VALIDATOR.FUNDING_PERIOD_PAGE,
  ...
  [config.routes.SSV.VALIDATOR.FUNDING_PERIOD_PAGE]:
    config.routes.SSV.VALIDATOR.ACCOUNT_BALANCE_AND_FEE,
  [config.routes.SSV.VALIDATOR.ACCOUNT_BALANCE_AND_FEE]:
    config.routes.SSV.VALIDATOR.SLASHING_WARNING,
  [config.routes.SSV.VALIDATOR.SLASHING_WARNING]:
    config.routes.SSV.VALIDATOR.CONFIRMATION_PAGE,
  [config.routes.SSV.VALIDATOR.DISTRIBUTION_METHOD.DISTRIBUTE_OFFLINE]: {
    [EValidatorFlowAction.OFFLINE_CLI]:
      config.routes.SSV.VALIDATOR.DISTRIBUTION_METHOD.UPLOAD_KEYSHARES,
    [EValidatorFlowAction.OFFLINE_DKG]:
...
const validatorRegistrationFlow = (currentRoute: string) => {
  const getMaxValidatorsCountPerRegistration = (clusterSize: number) => {
    ...
    return (
      Number(getLocalStorageFlagValue(MAXIMUM_VALIDATOR_COUNT_FLAG)) ||
      maximumCount
    );
  };
  ...
  const getBulkKeyShareComponent = (
    single: JSX.Element,
    multi: JSX.Element
  ): JSX.Element => {
