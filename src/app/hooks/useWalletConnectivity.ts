...
import {
  initContracts,
  resetContracts
} from '~root/services/contracts.service';
import notifyService from '~root/services/notify.service';
import { isChainSupported } from '~root/wagmi/config';
import {
  clearAllSettings,
  fetchAndSetFeeIncreaseAndPeriods,
  fetchAndSetMaxOperatorFee,
...
export const useWalletConnectivity = () => {
  ...
  const initiateWallet = async ({
    ...
    await Promise.all([
      ...
      await dispatch(fetchAndSetFeeIncreaseAndPeriods())
    ]);
    const [accountClusters, accountOperators] = await Promise.all([
    ...
    if (accountClusters.payload?.clusters.length) {
      dispatch(
        setStrategyRedirect(config.routes.SSV.MY_ACCOUNT.CLUSTER_DASHBOARD)
      );
      // @ts-expect-error payload is not typed
    } else if (accountOperators.payload?.operators.length) {
      dispatch(
        setStrategyRedirect(config.routes.SSV.MY_ACCOUNT.OPERATOR_DASHBOARD)
      );
    } else {
    ...
    await Promise.all([
      await dispatch(fetchAndSetMaxOperatorFee()),
