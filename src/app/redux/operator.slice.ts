...
export const fetchAndSetOperatorValidatorsLimit = createAsyncThunk(
  'operator/fetchAndSetOperatorValidatorsLimit',
  async () => {
    return await updateOperatorValidatorsLimit();
  }
);

export const fetchAndSetOperatorFeeInfo = createAsyncThunk(
  'operator/fetchAndSetOperatorFeeInfo',
  async (operatorId: number) => {
    return await syncOperatorFeeInfo(operatorId);
  }
);

export const fetchAndSetFeeIncreaseAndPeriods = createAsyncThunk(
  'operator/fetchAndSetFeeIncreaseAndPeriods',
  async () => {
    return await initFeeIncreaseAndPeriods();
  }
);
export const fetchAndSetMaxOperatorFee = createAsyncThunk(
  'operator/fetchAndSetMaxOperatorFee',
  async () => {
...
export const slice = createSlice({
  ...
  reducers: {
    ...
    selectOperator: (
      ...
      if (
        Object.values(state.selectedOperators).some(
          (operator: IOperator) => operator.id === action.payload.operator.id
        )
      ) {
        return;
      }
      state.selectedOperators[action.payload.selectedIndex.toString()] =
  ...
  extraReducers: (builder) => {
    builder.addCase(
      fetchAndSetOperatorValidatorsLimit.fulfilled,
    ...
    builder.addCase(fetchAndSetOperatorFeeInfo.fulfilled, (state, action) => {
      state.operatorApprovalBeginTime =
        action.payload.operatorApprovalBeginTime;
    ...
    builder.addCase(
      fetchAndSetFeeIncreaseAndPeriods.fulfilled,
      (state, action) => {
        state.getSetOperatorFeePeriod = action.payload.getSetOperatorFeePeriod;
        state.declaredOperatorFeePeriod =
          action.payload.declaredOperatorFeePeriod;
...
export const getSelectedOperatorsFee = (state: RootState) =>
  Object.values(state.operatorState.selectedOperators).reduce(
    (acc: number, operator: IOperator) => {
      return acc + fromWei(operator.fee);
    },
    0
  );
export const getOperatorProcessId = (state: RootState) =>
  state.operatorState.operatorProcessId;
export const getSelectedOperators = (state: RootState) =>
  state.operatorState.selectedOperators;
export const getClusterSize = (state: RootState) =>
  state.operatorState.clusterSize;
export const getMaxOperatorFeePerYear = (state: RootState) =>
  state.operatorState.maxOperatorFeePerYear;
export const hasEnoughSelectedOperators = (state: RootState) =>
  Object.values(state.operatorState.selectedOperators).length ===
  state.operatorState.clusterSize;
export const getOperatorValidatorsLimit = (state: RootState) =>
  state.operatorState.operatorValidatorsLimit;
