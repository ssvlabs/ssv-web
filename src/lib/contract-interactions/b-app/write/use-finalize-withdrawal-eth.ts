...
type Fn = ExtractAbiFunction<typeof BAppABI, "finalizeWithdrawalETH">;
const abiFunction = extractAbiFunction(BAppABI, "finalizeWithdrawalETH");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useFinalizeWithdrawalETH = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useFinalizeWithdrawalETH",
    bAppContractAddress,
  ]);
      ...
      .writeContractAsync(
        {
          ...
          functionName: "finalizeWithdrawalETH",
