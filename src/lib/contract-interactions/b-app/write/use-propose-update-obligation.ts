...
type Fn = ExtractAbiFunction<typeof BAppABI, "proposeUpdateObligation">;
const abiFunction = extractAbiFunction(BAppABI, "proposeUpdateObligation");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useProposeUpdateObligation = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useProposeUpdateObligation",
    bAppContractAddress,
  ]);
      ...
      .writeContractAsync(
        {
          ...
          functionName: "proposeUpdateObligation",
