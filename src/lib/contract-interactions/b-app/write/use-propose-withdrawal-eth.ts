...
type Fn = ExtractAbiFunction<typeof BAppABI, "proposeWithdrawalETH">;
const abiFunction = extractAbiFunction(BAppABI, "proposeWithdrawalETH");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useProposeWithdrawalETH = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useProposeWithdrawalETH",
    bAppContractAddress,
  ]);
    ...
    return mutation.writeContractAsync(
      {
        ...
        functionName: "proposeWithdrawalETH",
