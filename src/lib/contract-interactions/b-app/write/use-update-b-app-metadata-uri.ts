...
type Fn = ExtractAbiFunction<typeof BAppABI, "updateBAppMetadataURI">;
const abiFunction = extractAbiFunction(BAppABI, "updateBAppMetadataURI");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useUpdateBAppMetadataURI = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useUpdateBAppMetadataURI",
    bAppContractAddress,
  ]);
      ...
      .writeContractAsync(
        {
          ...
          functionName: "updateBAppMetadataURI",
