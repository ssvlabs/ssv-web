...
type Fn = ExtractAbiFunction<typeof BAppABI, "updateDisabledFeatures">;
const abiFunction = extractAbiFunction(BAppABI, "updateDisabledFeatures");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useUpdateDisabledFeatures = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useUpdateDisabledFeatures",
    bAppContractAddress,
  ]);
      ...
      .writeContractAsync(
        {
          ...
          functionName: "updateDisabledFeatures",
