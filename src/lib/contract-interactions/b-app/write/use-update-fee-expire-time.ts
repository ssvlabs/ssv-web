...
type Fn = ExtractAbiFunction<typeof BAppABI, "updateFeeExpireTime">;
const abiFunction = extractAbiFunction(BAppABI, "updateFeeExpireTime");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useUpdateFeeExpireTime = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useUpdateFeeExpireTime",
    bAppContractAddress,
  ]);
    ...
    return mutation.writeContractAsync(
      {
        ...
        functionName: "updateFeeExpireTime",
