...
type Fn = ExtractAbiFunction<typeof BAppABI, "updateFeeTimelockPeriod">;
const abiFunction = extractAbiFunction(BAppABI, "updateFeeTimelockPeriod");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useUpdateFeeTimelockPeriod = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useUpdateFeeTimelockPeriod",
    bAppContractAddress,
  ]);
      ...
      .writeContractAsync(
        {
          ...
          functionName: "updateFeeTimelockPeriod",
