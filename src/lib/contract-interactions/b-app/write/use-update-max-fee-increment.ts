...
type Fn = ExtractAbiFunction<typeof BAppABI, "updateMaxFeeIncrement">;
const abiFunction = extractAbiFunction(BAppABI, "updateMaxFeeIncrement");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useUpdateMaxFeeIncrement = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useUpdateMaxFeeIncrement",
    bAppContractAddress,
  ]);
    ...
    return mutation.writeContractAsync(
      {
        ...
        functionName: "updateMaxFeeIncrement",
