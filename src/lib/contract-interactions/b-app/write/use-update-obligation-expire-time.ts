...
type Fn = ExtractAbiFunction<typeof BAppABI, "updateObligationExpireTime">;
const abiFunction = extractAbiFunction(BAppABI, "updateObligationExpireTime");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useUpdateObligationExpireTime = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useUpdateObligationExpireTime",
    bAppContractAddress,
  ]);
    ...
    return mutation.writeContractAsync(
      {
        ...
        functionName: "updateObligationExpireTime",
