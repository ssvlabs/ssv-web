...
type Fn = ExtractAbiFunction<typeof BAppABI, "updateStrategyMetadataURI">;
const abiFunction = extractAbiFunction(BAppABI, "updateStrategyMetadataURI");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useUpdateStrategyMetadataURI = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useUpdateStrategyMetadataURI",
    bAppContractAddress,
  ]);
      ...
      .writeContractAsync(
        {
          ...
          functionName: "updateStrategyMetadataURI",
