...
type Fn = ExtractAbiFunction<typeof BAppABI, "withdrawETHSlashingFund">;
const abiFunction = extractAbiFunction(BAppABI, "withdrawETHSlashingFund");
// type State = "idle" | "confirming" | "mining" | "mined" | "error";

export const useWithdrawETHSlashingFund = () => {
  const { bAppContractAddress } = useSSVNetworkDetails();

  const wait = useWaitForTransactionReceipt<BAppEvent>([
    "useWithdrawETHSlashingFund",
    bAppContractAddress,
  ]);
    ...
    return mutation.writeContractAsync(
      {
        ...
        functionName: "withdrawETHSlashingFund",
