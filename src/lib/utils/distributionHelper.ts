...
export const distributionHelper = (networkId: number) => {
  ...
  let storeName =
    networkId === MAINNET_NETWORK_ID ? DISTRIBUTION : DISTRIBUTION_TESTNET;
  // @ts-ignore
  const enableDistributionTestingFlow = JSON.parse(
    getFromLocalStorageByKey(DEVELOPER_FLAGS.ENABLE_TEST_DISTRIBUTION_FLOW)
  );
