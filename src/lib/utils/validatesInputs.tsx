...
export const validatePublicKeyInput = (
  value: string,
  callback: React.Dispatch<ErrorObject>
): void => {
  ...
  if (value.length === 0) {
    ...
    response.errorMessage = (
      <>
        ...
        <LinkText
          text={'documentation.'}
          link={
            'https://docs.ssv.network/run-a-node/operator-node/installation#generate-operator-keys'
          }
        />{' '}
    ...
    response.errorMessage =
      'Operator key should contain only alphanumeric characters.';
  } else {
...
export const validateAddressInput = (
  ...
  fieldName: string = 'Operator address'
): void => {
  ...
  if (value.length === 0 && skipEmpty) {
    ...
    (value.length !== 40 && !value.startsWith('0x')) ||
    !isAddress(value)
  ) {
...
export const validateFeeInput = ({
  ...
  if (
    new Decimal(
      Number(value) / config.GLOBAL_VARIABLE.BLOCKS_PER_YEAR
    ).lessThan(config.GLOBAL_VARIABLE.MINIMUM_OPERATOR_FEE_PER_BLOCK)
  ) {
    response.shouldDisplay = true;
    const minimumFeePerYear =
      config.GLOBAL_VARIABLE.BLOCKS_PER_YEAR *
      config.GLOBAL_VARIABLE.MINIMUM_OPERATOR_FEE_PER_BLOCK;
...
export const validateFeeUpdate = ({
  ...
  // eslint-disable-next-line radix
    ...
    response.shouldDisplay = true;
    response.errorMessage =
      'You must set your operator as private before updating your fee to 0.';
  } else {
    response.errorMessage = '';
    response.shouldDisplay = false;
  }
