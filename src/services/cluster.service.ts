...
const extendClusterEntity = async (
  cluster: ICluster,
  ownerAddress: string,
  ...
  const balance = await getClusterBalance(
    cluster.operators,
    ownerAddress,
  ...
  const burnRate = await getClusterBurnRate(
    operatorIds,
    ownerAddress,
    clusterData
  );
...
const getClusterHash = (
  operators: (number | IOperator)[],
  ownerAddress: string
) => {
  let operatorsIds;
...
const getClusterBalance = async (
  ...
  const clusterData =
    ...
    (await getClusterData(
      getClusterHash(operators, ownerAddress),
      liquidationCollateralPeriod,
      minimumLiquidationCollateral
    ));
...
const getClusterNewBurnRate = (
  operators: IOperator[],
  newAmountOfValidators: number,
  networkFee: number
) => {
  const operatorsFeePerYear = operators.reduce(
    (acc: number, operator: IOperator) =>
      Number(acc) + Number(getFeeForYear(fromWei(operator.fee))),
    0
  );
  const operatorsFeePerBlock = new Decimal(operatorsFeePerYear)
    .dividedBy(config.GLOBAL_VARIABLE.BLOCKS_PER_YEAR)
    .toFixed()
    .toString();
  const networkFeePerBlock = new Decimal(networkFee).toFixed().toString();
  const clusterBurnRate =
    parseFloat(operatorsFeePerBlock) + parseFloat(networkFeePerBlock);
  return clusterBurnRate * newAmountOfValidators;
};

const isClusterLiquidated = async (
  operatorsIds: number[],
  ownerAddress: string,
  clusterData: any
): Promise<boolean> => {
  ...
  try {
    return await contract.isLiquidated(
      ownerAddress,
      sortedOperatorsIds,
      clusterData
    );
...
const getClusterBurnRate = async (
  operators: number[],
  ownerAddress: string,
  clusterData: any
): Promise<string> => {
...
const getClusterRunWay = (
  cluster: any,
  liquidationCollateralPeriod: number,
  minimumLiquidationCollateral: number
) => {
  const liquidationCollateral =
    liquidationCollateralPeriod / config.GLOBAL_VARIABLE.BLOCKS_PER_DAY;
  const burnRatePerDay =
    fromWei(cluster.burnRate) * config.GLOBAL_VARIABLE.BLOCKS_PER_DAY;
  ...
  return Math.max(
    (fromWei(cluster.balance) - liquidationCollateralCost) / burnRatePerDay,
    0
  );
};
...
const getClusterData = async (
  ...
  if (clusterData === null) {
    ...
    const isLiquidated = await isClusterLiquidated(
      Object.values(clusterData.operators),
      clusterData.ownerAddress,
      clusterData
    );
    const burnRate: string = await getClusterBurnRate(
      Object.values(clusterData.operators),
      clusterData.ownerAddress,
      clusterData
    );
