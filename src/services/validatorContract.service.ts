...
const removeValidators = async ({
  ...
  const clusterData = await getClusterData(
    getClusterHash(operators, accountAddress),
    liquidationCollateralPeriod,
    minimumLiquidationCollateral
  );
  ...
  return await transactionExecutor({
    contractMethod: isBulk
      ? contract.bulkRemoveValidator
      : contract.removeValidator,
    payload,
...
const exitValidators = async ({
  isContractWallet,
  validatorIds,
  operatorIds,
  isBulk,
  dispatch
}: IExitValidators): Promise<boolean> => {
  const payload = [validatorIds, operatorIds];
  const contract = getContractByName(EContractName.SETTER);
  return await transactionExecutor({
    contractMethod: isBulk
      ? contract.bulkExitValidator
