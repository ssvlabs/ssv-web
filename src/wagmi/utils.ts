...
type ContractMethod = (...args: unknown[]) => Promise<{
  hash: string;
  wait: () => Promise<
    WaitForTransactionReceiptReturnType & { events?: string }
  >;
}>;
...
export const getContractByName = <T extends EContractName>(
  ...
  switch (name) {
    ...
    case EContractName.GETTER: {
      return _getContractByName(name) as T extends EContractName.SETTER
        ? SetterContract
        : Contract;
    }
    case EContractName.SETTER:
    default:
      return getSetter() as T extends EContractName.SETTER
        ? SetterContract
        : Contract;
  }
};
